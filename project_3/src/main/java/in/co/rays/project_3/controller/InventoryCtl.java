...
public class InventoryCtl  extends BaseCtl {
 ...
 request.setAttribute("sm", map);
	...
	protected boolean validate(HttpServletRequest request) {
		...
		else if (request.getParameter("supplierName").length() > 50) {
			request.setAttribute("supplierName", " only 50 characters are allowed ");
		...
		if (DataValidator.isNull(request.getParameter("lastUpatedDate"))) {
			request.setAttribute("lastUpatedDate", PropertyReader.getValue("error.require", " lastUpatedDate"));
		...
		else if (!DataValidator.isQuentity(request.getParameter("quantity"))) {
			request.setAttribute("quantity", " quantity must be greater than 0 ");
			System.out.println(pass);
			pass = false;
		}
	...
	protected BaseDTO populateDTO(HttpServletRequest request) {
		...
		dto.setLastUpatedDate(DataUtility.getDate(request.getParameter("lastUpatedDate")));
