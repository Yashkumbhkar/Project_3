...
public class LoansCtl  extends BaseCtl{
	...
	protected void preload(HttpServletRequest request) {
		...
		LoansModelInt model1 = ModelFactory.getInstance().getLoansModel();
		try {
 ...
 request.setAttribute("sm", map);
	...
	protected boolean validate(HttpServletRequest request) {
		 ...
		 */		if (DataValidator.isNull(request.getParameter("intrestRate"))) {
			...
			request.setAttribute("intrestRate", " intrestRate must be between 1 an 100 ");
			System.out.println(pass);
			pass = false;
		}else if (Integer.parseInt(request.getParameter("intrestRate")) == 0) {
			request.setAttribute("intrestRate", " intrestRate must be contain 1 ");
			System.out.println(pass);
			pass = false;
		}
